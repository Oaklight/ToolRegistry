...
class TestExecuteToolCallsThread:
    def test_basic_execution(self):
        ...
        assert next(r for r in result if r.id == "call_1").result == "7"
    ...
    def test_async_tool(self):
        ...
        result = reg.execute_tool_calls([tc], execution_mode="thread")
        assert next(r for r in result if r.id == "call_1").result == "15"
...
class TestExecuteToolCallsProcess:
    def test_basic_execution(self):
        ...
        assert next(r for r in result if r.id == "call_1").result == "30"
...
class TestDisabledToolsRegression:
    def test_disabled_tool_returns_error(self):
        ...
        assert "disabled" in str(next(r for r in result if r.id == "call_1")).lower()
...
class TestTimeout:
    def test_timeout_enforcement(self):
        ...
        result = reg.execute_tool_calls([tc], execution_mode="thread")
        assert (
            "timed out" in str(next(r for r in result if r.id == "timeout_1")).lower()
        )
...
class TestConcurrencySafe:
    def test_unsafe_tools_run_sequentially(self):
        ...
        assert next(r for r in result if r.id == "a").result == "1"
        assert next(r for r in result if r.id == "b").result == "2"
...
class TestSetExecutionMode:
    def test_switch_mode(self):
        ...
        result = reg.execute_tool_calls([tc])
        assert next(r for r in result if r.id == "call_1").result == "2"
...
class TestContextInjection:
    def test_ctx_injected_in_thread_mode(self):
        ...
        result = reg.execute_tool_calls([tc], execution_mode="thread")
        assert "has_ctx=True" in next(r for r in result if r.id == "call_1").result
