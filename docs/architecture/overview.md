...
ToolRegistry 为**智能体开发者**设计 — 即构建 AI 智能体和 LLM 驱动应用的工程师，这些应用需要根据模型决策调用外部函数（工具）。如果你的应用使用任何 LLM 供应商的函数调用 / 工具调用功能，ToolRegistry 为你提供统一的工具注册、管理和执行方式。
...
ToolRegistry 管理整个生命周期：从多种来源注册工具、为任意 LLM 供应商生成 Schema、并发执行调用、以及恢复多轮对话消息。
...
- **生成**供应商特定格式的 Schema（OpenAI、Anthropic、Gemini）
- **执行**工具调用，支持可配置的并发后端
- **控制访问**，通过权限策略和元数据标签
- **组织**工具到命名空间中
...
ToolRegistry 通过 [llm-rosetta](https://pypi.org/project/llm-rosetta/) 为多个 LLM 供应商生成工具 Schema：
...
详见 [LLM 供应商](../usage/providers/openai_chat.md)章节的供应商特定集成指南。
