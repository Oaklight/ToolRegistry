...
description: 如何使用 build_assistant_messages、build_tool_result_messages 和 build_tool_call_messages 构建 LLM 对话历史
...
| 函数 | 层级 | 输入 | 输出 |
|------|------|------|------|
| `build_assistant_messages()` | 模块级 | `list[ToolCall]` | 包含工具调用请求的助手消息 |
| `build_tool_result_messages()` | 模块级 | `dict[str, str]` | 工具结果消息 |
...
| `"openai-responses"` | OpenAI Response API 格式 |
...
| 缺少 `tool_calls` 参数导致 Gemini 函数名错误 | `build_tool_result_messages()` | Gemini 格式下始终传入 `tool_calls` |
| 混合内容（文本 + 工具调用）被静默丢弃 | `build_assistant_messages()` | 自行从原始响应中保留非工具调用内容 |
