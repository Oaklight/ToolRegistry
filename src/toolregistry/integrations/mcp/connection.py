...
class MCPConnectionManager:
    ...
    # -- Sync API (shared async runtime) ----------------------------

    def call_tool_sync(self, name: str, arguments: dict[str, Any]) -> CallToolResult:
        """Call a tool synchronously via the shared :class:`AsyncRuntime`.

        The runtime's persistent daemon thread keeps MCP transport
        resources alive across calls.
